...
(function () {
  ...
  function creaLavoratoreOcr(indiceFonte) {
    ...
    if (indice >= FONTI_OCR.length) {
      return Promise.reject(new Error(
        'Il PDF è una scansione e per leggerla serve il riconoscimento del testo, ' +
        'che non riesco a caricare. Collegati a internet e riprova, oppure inserisci ' +
        'le righe a mano.'
      ));
    }
    var fonte = FONTI_OCR[indice];
    ...
    return preparazione.then(function () {
      return self.Tesseract.createWorker('eng', 1, {
    ...
    return creaLavoratoreOcr().then(function (lavoratore) {
      ...
      function prossima(numero) {
        ...
        return documento.getPage(numero).then(disegnaPagina).then(function (tela) {
          return lavoratore.recognize(tela, {}, { blocks: true }).then(function (esito) {
