...
(function () {
  ...
  /* Che cosa e' stato scelto davvero? Sull'iPhone capita spesso di prendere la
     foto del modulo, o l'allegato sbagliato, invece del PDF: meglio dirlo
     subito e con parole chiare che lasciar fallire pdf.js in inglese.
     Il "%PDF" non e' sempre il primo byte, quindi si cerca nell'intestazione. */
  function tipoDiFile(dati) {
    if (!dati || dati.length < 8) { return 'vuoto'; }
    var limite = Math.min(dati.length - 3, 1024);
    for (var i = 0; i < limite; i++) {
      if (dati[i] === 0x25 && dati[i + 1] === 0x50 &&
          dati[i + 2] === 0x44 && dati[i + 3] === 0x46) { return 'pdf'; }
    }
    if (dati[0] === 0xff && dati[1] === 0xd8) { return 'immagine'; }
    if (dati[0] === 0x89 && dati[1] === 0x50 && dati[2] === 0x4e && dati[3] === 0x47) {
      return 'immagine';
    }
    if (dati[4] === 0x66 && dati[5] === 0x74 && dati[6] === 0x79 && dati[7] === 0x70) {
      return 'immagine'; // HEIC dell'iPhone, e i filmati
    }
    if (dati[0] === 0x50 && dati[1] === 0x4b) { return 'office'; }
    if (dati[0] === 0xd0 && dati[1] === 0xcf) { return 'office'; }
    return 'altro';
  }

  var SPIEGAZIONE_FILE = {
    vuoto: 'Il file scelto è vuoto. Scaricalo di nuovo dalla mail e riprova.',
    immagine: 'Hai scelto un\'immagine, non un PDF. L\'app legge il PDF del modulo: se ' +
      'hai solo una foto o una scansione salvata come immagine, convertila prima in PDF ' +
      '(sull\'iPhone si può fare da File, con Stampa e poi Salva come PDF).',
    office: 'Hai scelto un documento Word, Excel o un file compresso, non un PDF. ' +
      'Apri il modulo e salvalo come PDF, poi carica quello.',
    altro: 'Questo file non è un PDF. Scegli il PDF del modulo Ship Pre-Arrival ' +
      'Security Information Form.'
  };

  /* pdf.js segnala i guai con eccezioni dal nome inglese: qui diventano frasi
     che dicono anche che cosa fare. */
  function erroreApertura(errore) {
    var nome = (errore && errore.name) || '';
    if (nome === 'PasswordException') {
      return new Error('Questo PDF è protetto da password. Aprilo inserendo la password, ' +
        'salvane una copia senza protezione e carica quella.');
    }
    if (nome === 'InvalidPDFException') {
      return new Error('Questo PDF è danneggiato o è stato scaricato a metà. Scaricalo ' +
        'di nuovo dalla mail e riprova.');
    }
    if (nome === 'MissingPDFException' || nome === 'UnexpectedResponseException') {
      return new Error('Non riesco a leggere il file. Copialo sul dispositivo e ' +
        'scegli di nuovo il file copiato.');
    }
    return errore;
  }
  ...
  function apriDocumento(datiPdf) {
    ...
    return pdfjsLib.getDocument(opzioni).promise.catch(function (errore) {
      throw erroreApertura(errore);
    });
  }
  ...
  function alFileScelto(file) {
    ...
    Promise.all([leggiFile(file), attesa(pdfPronto, 'il lettore di PDF')])
      .then(function (esiti) {
        var dati = new Uint8Array(esiti[0]);
        var tipo = tipoDiFile(dati);
        if (tipo !== 'pdf') { throw new Error(SPIEGAZIONE_FILE[tipo]); }
        return apriDocumento(dati);
      })
